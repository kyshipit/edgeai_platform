...
#define TEXT_SCORE 0.35

// 与 YOLO 的 rknn_app_context_t 字段不同，单独命名避免多适配器同编链接时 typedef 冲突。
typedef struct {
    ...
    int model_width;
    int model_height;
} ppocr_rknn_context_t;

typedef struct {
    ppocr_rknn_context_t det_context;
    ppocr_rknn_context_t rec_context;
...
int init_ppocr_model(const char* model_path, ppocr_rknn_context_t* app_ctx);

int release_ppocr_model(ppocr_rknn_context_t* app_ctx);

int inference_ppocr_det_model(ppocr_rknn_context_t* app_ctx, image_buffer_t* src_img, ppocr_det_postprocess_params* params, ppocr_det_result* out_result);

int inference_ppocr_rec_model(ppocr_rknn_context_t* app_ctx, image_buffer_t* src_img, ppocr_rec_result* out_result);
