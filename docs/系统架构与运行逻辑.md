...
| LLM 预加载时机 | `preload_on_startup` 在 Pipeline/YOLO Init **之前** | 先视觉后 LLM | 产品要求减首句延迟；代价是与视觉争 NPU——缺 `.rkllm` 时须 **stat 预检**，避免误触发 `rkllm_init` 拖垮 YOLO |
...
| 7 | `pipeline.Run()`；退出时 `TtsWorker`/`LlmWorker::Shutdown` |
...
| Active | 可对话；稳定后自动问候上 `AI>` |
| Grace | 短暂离开仍可能受理；超时回 Locked |

- **问候**：`TryAutoPromptOnStableFace` → `SetBannerLine`（静态 yaml）。
- **对话**：`SubmitUserPrompt` → `LlmWorker::SubmitPrompt` → `infer_thread_` 内 `rkllm_run`。
...
| `model.llm.enabled` | LlmWorker | 是否可对话 |
...
| 阶段 | 屏幕 | 终端 | 语音 TTS | 后台 |
|------|------|------|----------|------|
| 待机 | badge `yolo` 或 `none` | `SYS>` 就绪 | 静音 | idle |
